...
export default Greeting;`,
  ...
  },
  {
    ...
    content: [
      ...
      {
        p: "Первый параметр: Функция, которая должна выполниться при монтировании компонента и при каждом обновлении.",
      },
      {
        p: "Второй параметр: Массив зависимостей определяет, на что конкретно должен реагировать useEffect. Если массив пуст, useEffect выполняется только при монтировании и размонтировании. Если в массиве указаны определенные значения, то useEffect будет выполняться только тогда, когда эти значения изменяются.",
      },
      {
        c: `// Отработает как componentDidMount
useEffect(() => { console.log("componentDidMount") }, []) 
...
export default Example;`,
      ...
      {
        p: "В React хуки, такие как useEffect, выполняются при каждом рендере компонента, включая первый рендер после монтирования. Это особенность работы хуков, и они не повторяют точно поведение метода жизненного цикла componentDidMount из классовых компонентов.",
      },
      {
  ...
  },
  {
    ...
    content: [
      {
        p: "useContext - это хук React, который позволяет вам читать и подписываться на контекст из вашего компонента.",
      },
      {
        c: `const value = useContext(SomeContext);`,
      },
      {
        p: "Вызовите useContext на верхнем уровне вашего компонента для чтения и подписки на контекст.",
      },
      {
...
function MyComponent() {
    ...
    // ...
      ...
      {
        p: "SomeContext: Контекст, который вы ранее создали с помощью createContext. Сам контекст не хранит информацию, он только представляет тип информации, которую вы можете предоставить или прочитать из компонентов.",
      },
      {
        b: "Возвращает",
      },
...
function Button() {
    ...
    // ...
      ...
      {
        p: "useContext возвращает значение контекста для контекста, который вы передали. Чтобы определить значение контекста, React просматривает дерево компонентов и находит ближайшего поставщика контекста выше для данного контекста.",
      },
      {
        p: "Чтобы передать контекст Button, оберните его или один из его родительских компонентов в соответствующий провайдер контекста:",
      },
      {
        c: `function MyPage() {
    return (
...
function Form() {
    // ... renders buttons inside ...
      ...
      {
        p: "Не имеет значения, сколько слоев компонентов находится между провайдером и Button. Когда Button в любом месте внутри Form вызывает useContext(ThemeContext), он получит dark в качестве значения.",
      },
      {
        err: "useContext() всегда ищет ближайшего провайдера выше компонента, который его вызывает. Она ищет вверх и не рассматривает провайдеров в компоненте, из которого вы вызываете useContext().",
      },
      {
        h: "Обновление данных, переданных через контекст",
      },
      {
        p: "Часто требуется, чтобы контекст менялся с течением времени. Чтобы обновить контекст, объедините его с состоянием. Объявите переменную состояния в родительском компоненте и передайте текущее состояние как значение контекста провайдеру.",
      },
      {
        c: `function MyPage() {
    ...
    return (
      ...
      {
        p: "Теперь любой Button внутри провайдера будет получать текущее значение theme. Если вы вызовете setTheme для обновления значения theme, которое вы передаете провайдеру, все компоненты Button будут перерисованы с новым значением 'light'",
      },
    ],
    path: "useContext",
  },
  {
    ...
    content: [
      {
        p: "useMemo — это хук для оптимизации производительности, который позволяет кэшировать результаты вычислений и избегать ненужных пересчётов при каждом рендере. Он позволяет сохранять результат вычислений и повторно использовать его, если входные значения (зависимости) не изменились.",
      },
      {
        h: "Что принимает и что возвращает useMemo?",
      },
      {
        b: "Принимает useMemo(() => computeValue, [dependencies]):",
      },
      {
        p: "1. Функция – вычисляемое значение, которое нужно кэшировать.",
      },
      {
        p: "2. Массив зависимостей – список значений, при изменении которых вычисление будет выполняться заново.",
      },
      {
        b: "Возвращает:",
      },
      {
        p: "Мемоизированное (кэшированное) значение – результат работы переданной функции.",
      },
      {
        c: `const memoizedValue = useMemo(() => computeExpensiveValue(a, b), [a, b]);`,
      },
      {
        p: "Если a и b не изменились → возвращается старый закэшированный результат.",
      },
      {
        p: "Если a или b изменились → вызывается computeExpensiveValue(), и React сохраняет новый результат.",
      },
      {
        err: "Без useMemo – объект создаётся заново при каждом рендере!",
      },
      {
        c: `const Child = React.memo(({ user }) => {
...
function App() {
  ...
  return (
    <div>
      ...
      {
        p: " Child ререндерится каждый раз, потому что user={{ name: 'Павел' }} создаёт новый объект при каждом рендере.",
      },
      {
        c: `function App() {
  ...
  return (
    <div>
      ...
      {
        p: "Теперь user создаётся только один раз, и React.memo работает корректно.",
      },
    ],
  },
  {
    ...
    content: [
      {
        p: "Роутинг обеспечивает плавную навигацию между разделами приложения, не перезагружая страницу. Он позволяет обновлять только необходимые компоненты или данные, улучшая пользовательский опыт. Благодаря роутингу, приложение может динамически адаптироваться к параметрам маршрута, например, отображая данные определённого пользователя по его ID.",
      },
      {
        p: "React Router упрощает определение маршрутов и вводит новые возможности для вложенных маршрутов и переходов. ",
      },
      {
        h: "Основные компоненты",
      },
      {
        b: "BrowserRouter",
      },
      {
        p: "Этот компонент оборачивает ваше приложение и использует HTML5 историю для управления маршрутизацией.",
      },
      {
...
function App() {  
  return (  
    <BrowserRouter>  
      ...
      {
        c: `import { Routes, Route } from 'react-router';
<Routes>
  ...
  <Route path="/contact" element={<Contact />} />
      ...
      {
        p: "Определяет отдельный маршрут внутри Routes. Каждый Route имеет свой путь (path) и элемент (element), который будет отображаться, когда маршрут совпадает. ",
      },
      {
        c: `import { Routes, Route } from 'react-router';
<Routes>
  ...
  <Route path="/contact" element={<Contact />} />
      ...
      {
        p: "Также можно использовать дополнительные атрибуты, такие как exact для точного совпадения пути и (*) fallback для отображения компонента-заглушки, если не найден ни один из маршрутов. ",
      },
      {
        c: `import { Routes, Route } from 'react-router';
<Routes>
  ...
  <Route path="*" element={<NotFound />} />
      ...
      },
      {
        p:
          "Route с путем " *
          ", который соответствует любому несовпадающему маршруту. Он будет отображать компонент <NotFound />, если не найден ни один из определенных маршрутов.",
      },
      {
        b: "Link",
      },
      {
        p: "Используется для создания ссылок для навигации между маршрутами в приложении.",
      },
      {
        c: `import { Link } from 'react-router';
...
<Link to="/contact">Контакты</Link>`,
      ...
      {
        p: "Позволяет создавать активные ссылки по текущему активному маршруту.",
      },
      {
        c: `import { NavLink } from 'react-router';
<NavLink
  to="/faq"
  className={isActive =>
...
Messages
      ...
      {
        p: "В актуальной версии react-router для NavLink можно использовать свойство className или style, которые принимают функцию. Эта функция получает объект с информацией о ссылке, где ключевое значение – isActive. Если ссылка активна (то есть URL совпадает с тем, что указан в to), то isActive будет true, и, соответственно, можно задать нужный класс или стили.",
      },
      {
        b: "useNavigate",
      },
      {
        p: "Хук, который позволяет перенаправлять пользователя на другие маршруты в React Router.Когда ты используешь useNavigate, ты можешь получить функцию navigate и использовать ее для перенаправления пользователя на другой маршрут.",
      },
      {
...
const Home = () => {
  ...
  return (
    <div>
      ...
      {
        p: "Хук, который позволяет получать параметры из маршрута в React Router. Когда ты используешь useParams, ты можешь получить объект params, который содержит значения параметров из текущего маршрута.",
      },
      {
        c: `import { useParams } from 'react-router';
const UserProfile = () => {
  ...
  return (
    <div>
      ...
      {
        p: "В этом примере мы используем useParams, чтобы получить объект params. Здесь мы предполагаем, что в маршруте у нас есть параметр :username, и мы выводим его значение на странице профиля пользователя.",
      },
      { b: "useLocation" },
      {
        p: "Позволяет получать информацию о текущем URL и его параметрах в React Router. Когда ты используешь useLocation, ты можешь получить объект location, который содержит информацию о текущем URL",
      },
      {
        c: `import { useLocation } from 'react-router';
const Page = () => {
  ...
  return (
    <div>
      ...
      {
        p: "Служит контейнером для отображения компонентов, которые соответствуют маршрутам в твоем приложении.",
      },
      {
        c: `import { Outlet } from 'react-router';
const App = () => {
  return (
    <div>
      ...
      {
        p: "Outlet является местом, где будут отображаться компоненты, соответствующие текущему маршруту.Когда пользователь переходит по разным маршрутам, Outlet автоматически рендерит компоненты, связанные с этими маршрутами. Например, если текущий маршрут - '/about', то компонент, соответствующий этому маршруту, будет отображен внутри Outlet.",
      },
      {
        err: "Важно отметить, что Outlet должен быть расположен внутри компонента, который уже является частью маршрутизации, например, внутри компонента, который используется в маршруте.",
      },
      {
        p: "Например, если у тебя есть маршрут '/about', определенный с использованием компонента AboutPage, то Outlet должен быть включен внутри AboutPage для отображения компонентов, соответствующих вложенным маршрутам внутри '/about'. Использование Outlet позволяет тебе динамически отображать компоненты на основе текущего маршрута без необходимости явно указывать их в коде.",
      },
    ],
    path: "react-router",
  },
